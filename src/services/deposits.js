...
const getDep = (id) => db.get('SELECT * FROM deposits WHERE id = ?', id);
...
async function createDeposit(user, nominal, opts = {}) {
  ...
  if (opts.merchantRef) {
    const dup = await db.get('SELECT id FROM deposits WHERE user_id = ? AND merchant_ref = ?', user.id, opts.merchantRef);
  ...
  const dep = await db.get('SELECT * FROM deposits WHERE reff_id = ?', reffId);
  background(notify.depositPending(dep, user));
  return dep;
}
...
async function refreshDeposit(dep) {
  ...
  if (status === 'success') {
    ...
    const done = await db.tx(async (t) => {
      const r = await t.run(
        "UPDATE deposits SET status='success', fee=?, get_balance=?, paid_at=datetime('now') WHERE id=? AND status='pending'",
        fee, credit, dep.id
      );
      if (r.changes !== 1) return false;
      await changeBalance(t, dep.user_id, credit, 'deposit', `Deposit QRIS ${dep.reff_id}`);
      return true;
    });
    const fresh = await getDep(dep.id);
    if (done) {
      const user = await getUser(dep.user_id);
      background(notify.depositSuccess(fresh, user));
      background(sendMerchantWebhook(dep.user_id, 'deposit.success', publicView(fresh), fresh.callback_url));
    }
    return fresh;
  }
  ...
  if (FINAL_FAIL.includes(status)) {
    ...
    const r = await db.run("UPDATE deposits SET status=? WHERE id=? AND status='pending'", newStatus, dep.id);
    const fresh = await getDep(dep.id);
    if (r.changes === 1) background(sendMerchantWebhook(dep.user_id, `deposit.${newStatus}`, publicView(fresh), fresh.callback_url));
    return fresh;
  }
  return dep;
}
...
async function cancelDeposit(dep) {
  ...
  await db.run("UPDATE deposits SET status='cancel' WHERE id=? AND status='pending'", dep.id);
  const fresh = await getDep(dep.id);
  background(sendMerchantWebhook(dep.user_id, 'deposit.cancel', publicView(fresh), fresh.callback_url));
  return fresh;
}

const findForUser = (userId, id) =>
  db.get('SELECT * FROM deposits WHERE user_id = ? AND (reff_id = ? OR merchant_ref = ?)', userId, id, id);
